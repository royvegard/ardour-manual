...
% This was in the stackexchange listing but it makes the whole document's
% fonts look bad.
...
To open the \dialogue{Rhythm Ferret} dialogue, choose
\menu{Edit,Rhythm Ferret}.  This will open the dialogue box shown in
...
\begin{itemize}
\item \textbf{Use a monitor bus} --- as discussed in
  Section~\ref{sec:mute-and-solo}, this instructs Ardour to use a
  monitor bus, allowing the AFL/PFL features and the more advanced `monitor section' in the mixer window.
\item \textbf{Record monitoring handled by} --- this dictates how
  monitoring of incoming signals should be handled; this can be by the
  audio hardware (in which case Ardour does nothing), by JACK (in
  which case Ardour tells JACK how the monitoring should be) or by
  Ardour (in which case Ardour does things itself).  See
  Section~\ref{sec:monitoring} for a full discussion.
\end{itemize}
...
`Auto-connect master/monitor busses' will, if enabled, instruct Ardour to
automatically connect the outputs of the master and monitor busses to the first
$N$ `physical' JACK ports, where $N$ is the number of channels in
your master bus (e.g.\ 2 for stereo).  `Physical' JACK ports are
those that correspond to sound card outputs that get sound out into
the real world.

\todo{there is the monitor\_bus\_preferred\_bundle but no-one sets it}

`Connect track inputs' sets how new track inputs should be set up;
they can either be automatically connected to physical inputs or left
alone for you to connect yourself.

`Connect track and bus outputs' sets how new track and bus outputs
...
`Denormals' are very small floating point numbers which can be
...
\begin{figure}[ht]
\begin{center}
\includegraphics[scale=0.75]{diagrams/typical-jack-session.pdf}
\end{center}
