...
\begin{itemize}
\item \textbf{Select/move objects} (shortcut key `o') --- used to mark
...
\item \textbf{Select/move ranges} (shortcut key `r') --- used to mark
  ranges of time and to manipulate them.
\item \textbf{Zoom range} (shortcut key `z') --- this provides a mode whereby a time
  range can be dragged with the mouse, and the editor window will zoom
  to show that time range.
\item \textbf{Region gain} (shortcut key `g') --- used to edit audio
...
\item \textbf{Draw/edit MIDI notes} --- used to draw new MIDI notes
  ...
  there.
\item \textbf{Edit region contents} (shortcut key `e') --- this is a
  kind of `modifier' for the other tools.  When selected, it means
  that the other tools will operate on region \emph{contents} rather
...
represented within it.  Inside the summary, tracks are represented as
...
\item Clicking with \modtwo\ held down will move the playhead to the
  click position.
\item Clicking with \modtertiary\ held down will centre the editor's
  view at the click position.
\item Moving the mouse's scroll-wheel will scroll the editor's view.
\item Moving the mouse's scroll-wheel with \modone\ held down will
  zoom the editor's view in or out.
\item Moving the mouse's scroll-wheel with \modtwo\ held down will
...
The `processors' in the signal flow is a general name for a collection
of things which treat the signal in some way.  Ardour provides several
processors, some of which are for internal use and are not seen in the
mixer strip.  In addition, processors can also be plugins.  The
arrangement of processors is arbitrary, and there is no limit to how
many there can be.
...
allow you to alter its parameters to your heart's content.
Alternatively, right-clicking on the plugin and selecting
\menu{Controls,Show All Controls} adds the plugins controls directly
to the processor box.  This may be convenient for relatively simple
plugins, such as the plate reverb shown in the example screenshot.
...
to listen to signals that are coming intto the computer, perhaps as
they are being recorded.  Often, for example, one might be playing an
instrument for a recording and might want to hear what is being
recorded at the same time, perhaps along with some other
...
\item External monitoring --- this is where Ardour plays no role in
  ...
  Figure~\ref{fig:external-monitoring}.
...
The `p' control below the record enable will open a playlist menu when
clicked.  The menu offers various operations related to the track's
\emph{playlist}.  This, as you will recall, is simply a list of the
...
two, but generally speaking things are quite different.  This chapter
discusses Ardour's editing facilties; first the common ground between
audio and MIDI, and then the details of the specifics.


\section{Regions and sources}

Everything really begins with the Ardour concepts of \emph{regions}
and \emph{sources}.  A region is a snippet of audio or MIDI data of
any length.  Regions are the things that get displayed in the editor
window, and the things that we interact with directly when editing.  A
region gets its actual data (audio or MIDI) from a \emph{source}.
This is a thing which corresponds directly to a file on disk.  Regions
may share sources, so that, for example, copying a region many times
does not require copying of the source data.


\section{Basic region operations}

Figure~\ref{fig:audio-region} shows a typical audio region.
